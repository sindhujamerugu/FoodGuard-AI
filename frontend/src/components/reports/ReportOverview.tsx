...
export function ReportOverview({ report, showCustomer }: { report: FoodReport; showCustomer?: boolean }) {
  return (
    <div className="rounded-2xl border border-neutral-100 bg-white p-6 shadow-card">
      ...
      <div className="mt-5 flex aspect-video w-full items-center justify-center overflow-hidden rounded-xl bg-neutral-100">
        {report.image ? (
          <img src={report.image} alt={`Evidence for ${report.title}`} className="size-full object-cover" />
        ) : (
      ...
      <div className="mt-5 flex flex-wrap gap-x-6 gap-y-1 text-xs text-neutral-400">
